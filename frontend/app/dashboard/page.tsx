...
type Summary = { role: string; total_encounters: number; unique_patients: number; readmission_rate: number; high_risk_patients: number; model_status: string };
...
export default function DashboardPage() {
  ...
  return <main className="app-shell flex min-h-screen">
    ...
    <section className="min-w-0 flex-1 p-5 md:p-8">
      <header className="mb-7 flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-wider text-teal">{role}</p><h1 className="mt-1 text-3xl font-bold text-ink">Good day, {name.split(" ")[0]}</h1><p className="mt-1 text-slate-500">Hospital readmission and patient risk intelligence.</p></div><button onClick={signOut} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm lg:hidden">Sign out</button></header>
